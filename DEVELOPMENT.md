...
* Run `npm link @opuscapita/format-utils` at project's dir that's using the component to use local package
##### Build and watch the package
* Run `npm run watch[:cjs, :es, :umd]` to run dev builds in watch mode
##### Unlink local package
* Run `npm unlink @opuscapita/format-utils` at project's dir that's using the component
